...
#model = get_model(num_keypoints = 2, weights_path = 'ADTT/NN/keypointsrcnn_weights_5.pth')
#model.to(device)

KEYPOINTS_FOLDER_TEST = 'ADTT/NN/TRAIN'
...
for idx in range(len(next(os.walk( KEYPOINTS_FOLDER_TEST +"/Images" ))[2])):
    ...
    bboxes_g_truth = []
    for kps in targets[0]['boxes'].detach().cpu().numpy().astype(np.int32).tolist():
        
        bboxes_g_truth.append([kp for kp in kps])
        break
    ...
    ################################################################################################################
    save_image('ADTT/NN/CHECK/TRAIN', idx  , image, bboxes_g_truth, keypoints_g_truth)
    #visualize(idx  , image, bboxes, keypoints_g_truth)
